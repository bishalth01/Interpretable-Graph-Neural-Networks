...
#os.path.dirname("C:/Python39")
os.environ["LD_LIBRARY_PATH"]="/data/users2/bthapaliya/anaconda-main/anaconda3/lib"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batchSize', type=int, default=300, help='size of the batches')
    parser.add_argument('--dataroot', type=str, default='/data/users2/bthapaliya/BrainGNNABCDRegression/BrainGNNABCDAge/data/ABIDE_pcp/cpac/filt_noglobal/Output', help='root directory of the dataset')
    ...
    parser.add_argument('--save_path', type=str, default='./modelwithsch/', help='path to save model')
    ...
    fold = opt.fold
    writer = SummaryWriter(os.path.join('./logwithsch',str(fold)))
    ...
    def train(epoch):
        ...
        for data in train_loader:
            ...
            #loss_c = F.cross_entropy(output, data.y)
            data.y = data.y.to(torch.float32)
            
            loss_c = F.mse_loss(data.y, output.squeeze())
            ...
            writer.add_scalar('train/regression_loss', loss_c, epoch*len(train_loader)+step)
            ...
            loss_c = loss_c.to(torch.float32)
            loss = loss.to(torch.float32)


            loss_c.backward()
            loss_all += loss_c.item() * data.num_graphs
            optimizer.step()
    ...
    def test_r2(loader):
        ...
        return r2_sum / len(loader.dataset)
    
    def test_corr(loader):
        model.eval()
        corr_sum = 0
        for data in loader:
            data = data.to(device)
            ...
            pred = outputs[0]
            #correct += pred.eq(data.y).sum().item()
            corr_sum += pearsonr(data.y.cpu().detach().numpy(), pred.cpu().detach().numpy())

        return corr_sum / len(loader.dataset)
    ...
    def test_loss(loader,epoch):
        ...
        for data in loader:
            data = data.to(device)
            output, w1, w2, s1, s2= model(data.x, data.edge_index, data.batch, data.edge_attr,data.pos)
            loss_c = F.mse_loss(data.y, output)
            #loss_c = F.cross_entropy(output, data.y)
    ...
    def test_loss_reg(loader,epoch):
        print('testing...........')
        model.eval()
        loss_all = 0
        for data in loader:
    ...
    for epoch in range(0, num_epoch):
        ...
        val_loss = test_loss_reg(val_loader,epoch)
        time_elapsed = time.time() - since
        print('*====**')
        print('{:.0f}m {:.0f}s'.format(time_elapsed // 60, time_elapsed % 60))
        print('Epoch: {:03d}, Train Loss: {:.7f}, '
              'Train r2: {:.7f}, Test Loss: {:.7f}, Test r2: {:.7f}'.format(epoch, tr_loss,
                                                           tr_acc, val_loss, val_acc))

        writer.add_scalars('r2',{'train_r2':tr_acc,'val_r2':val_acc},  epoch)
        writer.add_scalars('Loss', {'train_loss': tr_loss, 'val_loss': val_loss},  epoch)
        writer.add_histogram('Hist_age/hist_s1', s1_arr, epoch)
        writer.add_histogram('Hist_age/hist_s2', s2_arr, epoch)
